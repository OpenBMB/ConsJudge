...
KILT_TASK = ['fever', 'aida', 't-rex', 'eli5', 'hotpotqa', 'wow', 'nq', 'marco', 'tqa', 'asqa']


def rougel_function(prediction, ground_truth):
    rouge = Rouge()
    # no normalization
    try:
...
def main():
    ...
    parser.add_argument('--model_name_or_path', type=str,
                        default='')
    parser.add_argument('--input_file', type=str, default='nq_psg.jsonl')
    ...
    parser.add_argument('--metric', type=str, default='nq')
    parser.add_argument('--top_n', type=int, default=2, help="number of paragraphs to be considered.")
    parser.add_argument('--task', type=str, default='nq')
    parser.add_argument('--batch_size', type=int, default=2)
    parser.add_argument('--user_chat_template', action='store_true')

    parser.add_argument('--replug', action='store_true', default=False)
    ...
    print("The parsed parameters are ：")
    ...
    if args.vllm:
        ...
        if args.use_lora:
            ...
            tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path, padding_side="left",
                                                      truncation_side="right", )
            model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, device_map='cuda',
                                                         trust_remote_code=True)
            ...
            if args.metric == "accuracy":
                if args.task in CHOICE_TASK:
                    ...
                    for pa in item["golds"]:
                        metric_result = 1.0 if pa in item["output"] or pa.lower() in item[
                            "output"] or pa.capitalize() in item["output"] else 0.0
    ...
    if output_path is not None:
        # When evaluating, it is necessary to save the outputs on the ASQA, Marco, and Wow datasets
        output_path = os.path.join(output_path, 'asqa_' + 'output.jsonl')
