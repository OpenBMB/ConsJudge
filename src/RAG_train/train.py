...
os.environ["WANDB_API_KEY"] = 'ad18ce74b06fb9b128a2d9525f076334169da9cb'  # 将引号内的+替换成自己在wandb上的一串值
...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="minicpm-2b-sft-bf16")
...
class DataArguments:
    train_data_path: str = field(
        default="train_shuffle.jsonl",
        metadata={"help": "Path to the training data."},
    )
    eval_data_path: str = field(
        default="dev_shuffle.jsonl",
...
def preprocessing(example, args, tokenizer):
    ...
    if example['data_type'] not in ['rag_bench']:
        if 'passage' in example:
            psgs = example['passage'][:args.top_n]
        else:
            psgs = example['gold_passages'][:args.top_n]
        psg_list = []
